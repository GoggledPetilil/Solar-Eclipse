...
def pbDisplayNameWindow(params)
  name         = params[1]
  color	       = params[0].windowskin.get_pixel(64, 96)
  isDark       = isDarkWindowskin(params[0].windowskin) #params[2]  params[2]
...
def pbMessageDisplay(msgwindow,message,letterbyletter=true,commandProc=nil)
  ...
  loop do
    ...
    if Input.trigger?(Input::USE) || Input.trigger?(Input::BACK)
      if msgwindow.busy?
        pbSEPlay("GUI message",80) if msgwindow.pausing?
        msgwindow.resume
      else
